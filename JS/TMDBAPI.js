...
function motd(){
  ...
  var motdFooter = ["Stay a while and listen!","Did somebody say [Thunderfury, Blessed Blade of the Windseeker]","Союз нерушимый республик свободных...","NORMAL SUMMON CIR, SPECIAL SUMMON ALICH, SPECIAL SUMMON BARBAR, OVERLAY INTO DANTE, DANTE EFF DETACH ALICH MILL 3, MILL SCARM, FIENDISH RHINO WARRIOR, POT OF DESIRES, LINK SUMMON UNDERCLOCK TAKER, DANTE EFF ADD BACK CIR, CIR EFFECT SPECIAL SUMMON DANTE, OVERLAY INTO BEATRICE, DISCARDING FARFA, FARFA EFF BANISH OPPONENTS BLUE-EYES WHITE DRAGON, SET TORRENTIAL TRIBUTE, PASS.","You're pretty good","Stop! You violated the law!","Some here in Helgen call you a hero, but a hero doesn't use the power like the voice to murder his king and usurp his throne. [muffled grunts] You started this war! Plunged Skyrim into chaos and now the empire is going to put you down and restore the peace!","Watch those wrist rockets!"]
  document.getElementById("Title").innerHTML = motdArray[Math.floor((Math.random() * motdArray.length-1) + 1)];
  document.getElementById("tatum").innerHTML = motdFooter[Math.floor((Math.random() * motdArray.length-1) + 1)];
